...
sitehealth crawl https://example.com --max-pages 300 --report-file crawl.txt

# 2) Validate the sitemap (auto-discovered) and check every URL's status + hreflang
sitehealth sitemap https://example.com --report-file sitemap.txt

# 3) Just list what's in the sitemap, no status checks
sitehealth sitemap https://example.com --no-check

# 4) Point at an explicit sitemap index
sitehealth sitemap https://example.com --sitemap https://example.com/sitemap_index.xml
```
